...
import { FaHeart, FaBookOpen, FaUsers, FaHandsHelping } from "react-icons/fa";
...
const donationCards = [
  {
    icon: FaBookOpen,
    title: "Enseignement",
    description: "Financez la production de contenus spirituels",
  },
  {
    icon: FaUsers,
    title: "Missions",
    description: "Soutenez l'évangélisation et les croisades",
  },
  {
    icon: FaHandsHelping,
    title: "Actions Sociales",
    description: "Participez à l'aide des familles et personnes vulnérables",
  },
];
...
export default function DonationSection() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % donationCards.length);
    }, 3000);
      ...
      <div className="max-w-7xl mx-auto px-6">
            ...
            {donationCards.map((_, index) => (
              <div
                ...
                className={`w-5 h-5 rounded-full transition-all duration-500 ${
                  activeIndex === index ? "bg-[#0B4A87]" : "bg-slate-300"
                }`}
              />
            ))}
          </div>
        ...
        <div className="mt-24 flex flex-col lg:flex-row justify-center items-center gap-12">
          {visibleCards.map((card) => {
            ...
            return (
              <div key={card.title} className="relative animate-fadeIn">
                {/* ICON BOX */}
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 w-24 h-24 bg-white rounded-[22px] shadow-xl flex items-center justify-center z-10">
                  <Icon size={36} className="text-[#071F5A]" />
                </div>
        ...
        {/* BOUTON DON → page /donations */}
        <div className="flex flex-col items-center mt-20 gap-4">
          <Link
            to="/donations"
            className="bg-[#E7B116] hover:bg-[#d7a10f] transition-all duration-300 rounded-full px-10 py-4 flex items-center gap-5 shadow-lg hover:scale-105"
          >
            <FaHeart size={28} className="text-[#071F5A]" />
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mt-2">
            {[
              { label: "Wave", color: "#1B9CE3" },
              { label: "Orange Money", color: "#FF7900" },
              { label: "Moov", color: "#00AEEF" },
          ...
          <p className="text-xs text-gray-400">Paiements sécurisés via CinetPay 🔒</p>
        </div>
