...
function CelluleModal({ onClose }) {
  ...
  const labelCls = "block text-xs font-semibold text-white/60 mb-1 uppercase tracking-wide";
  const errCls   = "text-red-400 text-xs mt-1";

  return (
    <motion.div
      className="fixed inset-0 z-50 flex items-center justify-center px-4"
      style={{ backgroundColor: "rgba(2,8,20,0.75)", backdropFilter: "blur(6px)" }}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      onClick={onClose}
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: "radial-gradient(circle, rgba(229,177,14,0.18) 0%, transparent 70%)" }}
        />

        {/* En-tête */}
        <div className="flex items-center justify-between px-5 sm:px-8 pt-6 sm:pt-8 pb-5 sm:pb-6">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 rounded-full bg-[#25D366] flex items-center justify-center shadow-lg shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full flex items-center justify-center text-white/50 hover:text-white hover:bg-white/10 transition-all duration-200 shrink-0 ml-2"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex gap-2 px-5 sm:px-8 mb-6 sm:mb-8">
          {[1, 2].map((n) => (
            <div
              key={n}
              className="flex-1 h-1 rounded-full transition-all duration-500"
              style={{ background: step >= n ? "#E5B10E" : "rgba(255,255,255,0.12)" }}
            />
          ))}
        </div>
        ...
        <div className="px-5 sm:px-8 pb-6 sm:pb-8">
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              ...
              >
                <p className="text-white font-bold text-base sm:text-lg mb-5 sm:mb-6">
                  Tes informations
                </p>
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4">
                  <div>
                    ...
                    <input className={inputCls("prenom")} placeholder="Marie" value={form.prenom} onChange={set("prenom")} />
                    {errors.prenom && <p className={errCls}>{errors.prenom}</p>}
                  </div>
                  <div>
                    <label className={labelCls}>Nom</label>
                    <input className={inputCls("nom")} placeholder="Kouassi" value={form.nom} onChange={set("nom")} />
                    {errors.nom && <p className={errCls}>{errors.nom}</p>}
                  </div>
                </div>
                ...
                <div className="mb-6 sm:mb-8">
                  ...
                  <input className={inputCls("ville")} placeholder="Abidjan" value={form.ville} onChange={set("ville")} />
                </div>

                <button
                  onClick={goStep2}
                  className="w-full h-12 rounded-2xl font-bold text-[#071F5A] flex items-center justify-center gap-2 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] text-sm sm:text-base"
                  style={{ background: "#E5B10E", boxShadow: "0 0 24px rgba(229,177,14,0.4)" }}
                >
            ...
            {step === 2 && (
              ...
              >
                ...
                <div className="mb-6 sm:mb-8">
                  <label className={labelCls}>Sujet de prière (facultatif)</label>
                  <input
                ...
                <button
                  onClick={submitForm}
                  className="w-full h-12 rounded-2xl font-bold text-[#071F5A] flex items-center justify-center gap-2 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] mb-3 text-sm sm:text-base"
                  style={{ background: "#E5B10E", boxShadow: "0 0 24px rgba(229,177,14,0.4)" }}
                >
  ...
  return (
    <section className="bg-[#F8F8F8] overflow-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center bg-[#071F5A] overflow-hidden" id="cellule">
        ...
        <div className="absolute inset-0 backdrop-blur-[10px] bg-gradient-to-br from-[#0B63CE]/25 via-[#071F5A]/20 to-[#0B63CE]/15" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(11,99,206,0.35),transparent_40%)]" />
        ...
        <motion.div
          ...
          className="absolute w-[400px] sm:w-[800px] h-[400px] sm:h-[800px] rounded-full bg-[#E5B10E] blur-[220px] opacity-40"
        />
        ...
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none z-10">
          <motion.div
            animate={{ x: [0, 500, 900, 500, 0, -500, -900, -500, 0], y: [0, -200, 0, 200, 0, -200, 0, 200, 0], rotateZ: [-8, 5, -5, 8, -8], scale: [1, 1.05, 1, 1.08, 1] }}
            transition={{ duration: 40, repeat: Infinity, ease: "linear" }}
            className="relative w-[220px] sm:w-[320px] h-[160px] sm:h-[220px]"
            style={{ perspective: "1200px" }}
          >
            <div className="absolute inset-0 rounded-full bg-yellow-300/20 blur-[80px] scale-125" />
            <div className="absolute inset-0" style={{ transformStyle: "preserve-3d" }}>
              <motion.div animate={{ rotateY: [0, -20, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} className="absolute left-0 top-0 w-1/2 h-full bg-[#4a2f1a] rounded-l-xl shadow-2xl" style={{ transformOrigin: "right center" }} />
              <motion.div animate={{ rotateY: [0, 20, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }} className="absolute right-0 top-0 w-1/2 h-full bg-[#4a2f1a] rounded-r-xl shadow-2xl" style={{ transformOrigin: "left center" }} />
              {[...Array(5)].map((_, i) => (
                <motion.div
                  key={i}
                  ...
                  style={{ transformOrigin: "left center", transformStyle: "preserve-3d", zIndex: 20 - i }}
                >
                  <div className="p-2 sm:p-4 text-[6px] sm:text-[8px] text-gray-400 leading-3">
                    Le Seigneur est mon refuge et ma force, un secours qui ne manque jamais dans la détresse.
                  </div>
                </motion.div>
              ))}
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 text-center">
          <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 4, repeat: Infinity }}>
            <FaHandsPraying className="mx-auto text-5xl sm:text-7xl text-[#F0B51B]" />
          </motion.div>

          <motion.div
            animate={{ y: [0, -20, 10, -15, 0], x: [0, 10, -10, 5, 0], rotate: [0, 1, -1, 0.5, 0] }}
          ...
          >
            <motion.h1
              ...
              className="text-white text-4xl sm:text-5xl md:text-7xl font-black mt-6 sm:mt-8 drop-shadow-[0_0_25px_rgba(255,255,255,0.3)] leading-tight"
            >
              Bienvenue à
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 mt-10 sm:mt-12 px-2">
            <button
              onClick={() => setModalOpen(true)}
              className="bg-[#E5B10E] text-[#071F5A] px-8 py-4 rounded-2xl font-bold hover:scale-105 transition-all shadow-[0_0_25px_rgba(255,80,0,0.8),0_0_50px_rgba(255,140,0,0.7),0_0_80px_rgba(255,215,0,0.5)] text-sm sm:text-base"
            >
              Intégrer la cellule
            </button>
            <NavLink
              to="/contact"
              className="border-2 border-white text-white px-8 py-4 rounded-2xl font-bold hover:bg-white hover:text-[#071F5A] transition-all duration-300 text-sm sm:text-base text-center"
            >
              Nous contacter
            </NavLink>
          </div>
        ...
        <AnimatePresence>
          {modalOpen && <CelluleModal onClose={() => setModalOpen(false)} />}
        </AnimatePresence>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-10 sm:gap-20 items-center">
            ...
            <div className="relative mt-6 lg:mt-0">
              <div className="absolute -top-4 -right-4 sm:-top-6 sm:-right-6 w-full h-full border-4 border-[#E5B10E] rounded-[24px] sm:rounded-[40px]" />
              <img
                src="fid2.png"
                alt="Notre histoire"
                className="relative rounded-[24px] sm:rounded-[40px] h-[280px] sm:h-[500px] w-full object-cover shadow-[0_20px_80px_rgba(0,0,0,0.12)]"
              />
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-8">
            {[
              ...
              <div key={i} className="bg-white p-7 sm:p-10 rounded-[24px] sm:rounded-[32px] shadow-xl">
                {item.icon}
                <h3 className="mt-4 sm:mt-6 text-2xl sm:text-3xl font-bold text-[#071F5A]">{item.title}</h3>
                <p className="mt-3 sm:mt-4 text-gray-600 text-sm sm:text-base">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
            {stats.map((stat, index) => (
              <div key={index} className="bg-white p-5 sm:p-10 rounded-[20px] sm:rounded-[32px] text-center shadow-xl">
                <p className="text-xs sm:text-sm font-semibold text-[#071F5A]/60 uppercase tracking-widest mb-2">
                  {index === 0 ? "Plus de" : index === 1 ? "Plus de" : index === 2 ? "Plus de" : "Plus de"}
                </p>
                <h3 className="text-3xl sm:text-5xl font-bold text-[#E5B10E]">
                  {stat.value}
                </h3>
                <p className="mt-2 sm:mt-4 text-[#071F5A] font-semibold text-sm sm:text-base">
                  {stat.label}
                </p>
              </div>
            ))}
      ...
      <section className="py-16 sm:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center">
            <h2 className="text-3xl sm:text-5xl font-bold text-[#071F5A] leading-tight">
              Nos valeurs fondamentales
            </h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 sm:gap-8 mt-10 sm:mt-16">
            {values.map((value, index) => (
              <div
                key={index}
                className="p-7 sm:p-10 rounded-[24px] sm:rounded-[32px] shadow-xl hover:-translate-y-3 transition-all duration-500 bg-[#F8F8F8]"
              >
                <div className="text-[#E5B10E]">{value.icon}</div>
                <h3 className="mt-4 sm:mt-6 text-2xl sm:text-3xl font-bold text-[#071F5A]">{value.title}</h3>
                <p className="mt-3 sm:mt-4 text-gray-600 text-sm sm:text-base">{value.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-24 bg-[#071F5A]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-white text-3xl sm:text-5xl font-bold leading-tight">
            Pourquoi rejoindre Prayer win ?
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-10 sm:mt-16">
            {reasons.map((reason, index) => (
              <div
                key={index}
                className="bg-white/10 backdrop-blur-md rounded-2xl sm:rounded-3xl p-5 sm:p-6 text-white flex items-center gap-3 sm:gap-4 text-left"
              >
                <CheckCircle2 className="text-[#E5B10E] shrink-0" size={24} />
          ...
          <div className="mt-10 sm:mt-12">
            <button
              onClick={() => setModalOpen(true)}
              className="bg-[#E5B10E] text-[#071F5A] px-8 sm:px-10 py-4 rounded-2xl font-bold transition-all duration-300 hover:scale-105 shadow-[0_0_25px_rgba(255,80,0,0.8),0_0_50px_rgba(255,140,0,0.7),0_0_80px_rgba(255,215,0,0.5)] text-sm sm:text-base"
            >
              🔥 Intégrer la cellule
            </button>
          </div>
